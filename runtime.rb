...
def watch_process(info)
	...
	process.attach(wait: true) do
		begin
			output = LXC.run_command('/etc/init.d/compute status')
			puts info[:container] + ' ' + output
			sleep 1
		end until output == 'Running'
	end
	...
	puts info[:container] + " ssend start"
	$started = true
	begin
